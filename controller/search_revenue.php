<?php
    ...
    $details = $get_revenue->fetch_general_revenueDate($from, $to);
    $n = 1;
?>
...
<div class="all_modes">
    ...
    $cashs = $get_cash->fetch_sum_2dateCond('payments', 'amount_paid', 'payment_mode', 'date(start_dates)', $from, $to, 'Cash');
    if(gettype($cashs) === "array"){
        foreach($cashs as $cash){
            echo "<p class='sum_amount' style='background:var(--otherColor); border-radius:15px; box-shadow:1px solid #cdcdcd;border:1px solid #fff'><strong>Cash</strong>: ₦".number_format($cash->total, 2)."</p>";
        }
    }
    //get POS
    $get_pos = new selects();
    $poss = $get_pos->fetch_sum_2dateCond('payments', 'amount_paid', 'payment_mode', 'date(start_dates)', $from, $to, 'POS');
    if(gettype($poss) === "array"){
        foreach($poss as $pos){
            echo "<p class='sum_amount' style='background:var(--secondaryColor); border-radius:15px; box-shadow:1px solid #cdcdcd;border:1px solid #fff'><strong>POS</strong>: ₦".number_format($pos->total, 2)."</p>";
        }
    }
    //get transfer
    $get_transfer = new selects();
    $trfs = $get_transfer->fetch_sum_2dateCond('payments', 'amount_paid', 'payment_mode','date(start_dates)', $from, $to, 'Transfer');
    if(gettype($trfs) === "array"){
        foreach($trfs as $trf){
            echo "<p class='sum_amount' style='background:var(--primaryColor);border-radius:15px; box-shadow:1px solid #cdcdcd;border:1px solid #fff'><strong>Transfer</strong>: ₦".number_format($trf->total, 2)."</p>";
        }
    }
    // get sum
   ...
   /*  $get_credit = new selects();
    $credits = $get_credit->fetch_sum_2date2Cond('payments', 'amount_due', 'date(start_dates)', 'payment_mode', 'store', $from, $to, 'Credit', $store);
