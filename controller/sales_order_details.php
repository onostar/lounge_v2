    ...
    <div class="close_stockin add_user_form" style="width:20%; margin:0; box-shadow:none;background:none;">
        <section class="addUserForm">
            <div class="inputs">
                ...
                <!-- get add order status -->
                 <?php
                    $get_stat = new selects();
                    $stats = $get_stat->fetch_details_condGroup('sales', 'invoice', $invoice, 'add_order');
                    foreach($stats as $stat){
                        $status = $stat->add_order;
                    }
                    if($status == 1){
                ...
                <div class="data" style="width:100%">
                    <button onclick="printSalesOrder('post_ticket_add_order.php')" style="background:green; padding:10px; border-radius:15px;font-size:.8rem; box-shadow:2px 2px 2px #c4c4c4;">Save Order <i class="fas fa-print"></i></button>
                </div>
                <?php }else{?>
                    <div class="data" style="width:100%">
                    <button onclick="printSalesOrder('post_ticket.php')" style="background:green; padding:10px; border-radius:15px;font-size:.8rem; box-shadow:2px 2px 2px #c4c4c4;">Save Order <i class="fas fa-print"></i></button>
                </div>
                <?php }?>
            </div>
        </section>
